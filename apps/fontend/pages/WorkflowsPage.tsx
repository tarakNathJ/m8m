...
const WorkflowCard = ({
  ...
  const getTriggerInfo = () => {
    const triggerNode = Object.values(wf.nodes || {}).find(
      (node) => node.type === NodeType.Trigger
    );

    if (
      triggerNode ||
      Object.values(wf.nodes || {}).some((n) => n.type === NodeType.HttpRequest)
    ) {
  ...
  const edit_workflow = async () => {
    console.log("Editing workflow with ID:", wf.id);

    const responce = await api_init.get(
      `/api/workflow/get-workflow-data/${wf.id}`
    );
    ...
    if (responce.data.success) {
      ...
      sessionStorage.setItem(workFlow_id, wf.id);
      navigate("/editor/" + wf.id);
    }
  };

  const delete_workflow = async () => {
    try {
      ...
      <div className="flex justify-between items-start">
        <div>
          ...
          {triggerInfo?.tag === "WEBHOOK" && (
            <p className="font-mono text-xs text-gray-400 mt-2 bg-[#1c1e20] inline-block px-2 py-1 rounded"></p>
          )}
        </div>
          ...
          <div className="flex items-center gap-4">
            <div className="relative">
              ...
              <input
                ...
                className="bg-[#2a2d30] border border-gray-700 rounded-lg pl-10 pr-4 py-2 focus:outline-none focus:ring-2 focus:ring-[#4295f1] transition-all"
              />
            </div>
